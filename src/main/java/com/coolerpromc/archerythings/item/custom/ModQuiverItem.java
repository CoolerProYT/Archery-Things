    ...
    public InteractionResult use(Level level, Player player, InteractionHand hand) {
        if (!player.isCrouching()){
            if (player instanceof ServerPlayer serverPlayer){
                serverPlayer.openMenu(new SimpleMenuProvider((i, inventory,player1) -> new QuiverMenu(i, inventory, player1, hand), Component.translatable("item.archerythings.quiver")),
                        buf -> {
