...
public class QuiverScreen extends AbstractContainerScreen<QuiverMenu> {
    ...
    public boolean keyPressed(KeyEvent event) {
        if (event.key() >= 49 && event.key() <= 57){
            ...
            ClientPacketDistributor.sendToServer(new ServerBoundSelectQuiverSlotPacket(key, this.menu.hand == null ? Optional.empty() : Optional.of(this.menu.hand)));
            return true;
        }
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double scrollX, double scrollY) {
        if (scrollY == -1){
            ...
            this.menu.broadcastChanges();
            ClientPacketDistributor.sendToServer(new ServerBoundSelectQuiverSlotPacket(selected, Optional.ofNullable(this.menu.hand)));
        }
        if (scrollY == 1){
