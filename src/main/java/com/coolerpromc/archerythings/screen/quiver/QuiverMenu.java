...
public class QuiverMenu extends AbstractContainerMenu {
    ...
    public QuiverMenu(int id, Inventory inventory, Player player, @Nullable InteractionHand hand) {
        super(ModMenuTypes.QUIVER_MENU, id);
        if (hand != null && player.getItemInHand(hand).is(ModItems.QUIVER)){
        ...
        else if (hand != null && player.getItemInHand(hand).has(ModDataComponents.STORED_QUIVER)){
            quiver = player.getItemInHand(hand).get(ModDataComponents.STORED_QUIVER).stack();
        }
    ...
    public int selected(){
        return quiver.getOrDefault(ModDataComponents.SELECTED, 0);
    }
}
