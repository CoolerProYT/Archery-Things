...
import com.coolerpromc.archerythings.ArcheryThingsClient;
...
public class QuiverLayer<S extends HumanoidRenderState, M extends HumanoidModel<S>> extends RenderLayer<S, M> {
    ...
    public void submit(PoseStack poseStack, SubmitNodeCollector nodeCollector, int packedLight, S renderState, float yRot, float partialTick) {
        ...
        else if (renderState.getRenderData(ArcheryThingsClient.QUIVER_STACK) != null && renderState.getRenderData(ArcheryThingsClient.QUIVER_STACK).getItem() instanceof ModQuiverItem) {
            ItemStack quiver = renderState.getRenderData(ArcheryThingsClient.QUIVER_STACK);
            float z = 0.06875F;
            float z1 = -0.04875F;
            float arrowZ = 0.15f;
            float arrowZ1 = 0.25f;
