...
import { data } from '../archerythings'
import ItemSlot from './ItemSlot.vue'

const props = defineProps<{ id: string }>()

const recipe = computed(() => data.recipes.find((r) => r.id === props.id || r.id === `archerythings:${props.id}`))
...
<template>
  <div v-if="recipe" class="at-recipe">
    <div class="grid">
    ...
    <ItemSlot :id="recipe.result.id" :count="recipe.result.count" :size="44" label />
  </div>
  <p v-else class="at-muted">Recipe {{ id }} not found.</p>
</template>
